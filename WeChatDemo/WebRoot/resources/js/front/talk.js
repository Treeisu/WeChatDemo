...
//外层添加$()表示此函数是jQuery对象；
$(function(){
	render();//自定义函数进行页面初始化(加载滚动条等等)；
//	debugger;
	var content = "您好，很高兴为您服务！<br/>回复[查看]收取更多精彩详细内容。";
	content += "<br/>回复[更多]可以查看所有可用的指令。";
	content +="<br/><a href=''>感谢您的关注!</a>";
	// 添加公众号的开场白
...
function send() {
	...
	if(!content) {//!content表示的是当content为null，或者空格，或者其他不符合逻辑的时候就为真；
		alert("请输入内容！");
		return;
	}
	$.ajax({
		url : $("#basePath").val() + "reply.do",
		type : "POST",
		dataType : "text",
		timeout : 10000,
		data : {"content":content},//ajax请求带上的参数
		success : function (data) {
			appendDialog("talk_recordboxme","游客001",content);
